...
switch postlayer.type
    ...
    case 'bn'  %如果是bn，则前后两层特征图数目相同（各自相连），先求zscore的残差,再直接传递
        ...
        for i = 1 : convlayer.featuremaps  %当前层的特征图的个数
            ...
            convlayer.delta{i,1} = znorm_delta;
        end
    case 'fc' %如果是fc，表示当前层存在隐含的光栅层，需先进行反矢量化，直接传递
        [height, width, batchnum] = size(convlayer.a{1}); %取前一层特征map尺寸
        maparea = height * width;
        for i = 1 : convlayer.featuremaps  %当前层的特征图的个数
            convlayer.delta{i,1} = reshape(postlayer.delta((i - 1) * maparea + 1: i * maparea, :), height, width, batchnum); %反矢量化
        end
    case 'pool' %如果是pool，则前后两层特征图数目相同（各自相连），先上采样，再直接传递
        for i = 1 : convlayer.featuremaps  %当前层的特征图的个数
            convlayer.delta{i,1} = expand(postlayer.delta{i,1}, [postlayer.stride(1),postlayer.stride(2),1]) .* postlayer.maxPos{i,1}; %上采样
        end
    case 'conv'  %如果是卷积层，需先进行反卷积，再直接传递
        for i = 1 : convlayer.featuremaps  %当前层的特征图的个数
            z = zeros(size(convlayer.a{1}));
            ...
            convlayer.delta{i,1} = z;  %直接传递
        end
    case 'deconv'  %如果是转置卷积层，需先进行反卷积，再直接传递
         for i = 1 : convlayer.featuremaps  %当前层的特征图的个数
            z = zeros(size(convlayer.a{1}));  %临时变量
            for j = 1 : postlayer.featuremaps %后一层特征图的个数
